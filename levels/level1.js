const bgConfig = {
  ...
  layers: {
    ...
    third: {
      one: 'img/5_background/layers/3_third_layer/1.png',
      two: 'img/5_background/layers/3_third_layer/2.png'
    },
    second: {
      one: 'img/5_background/layers/2_second_layer/1.png',
      two: 'img/5_background/layers/2_second_layer/2.png'
    },
    first: {
      one: 'img/5_background/layers/1_first_layer/1.png',
      two: 'img/5_background/layers/1_first_layer/2.png'
    }
  }
};
...
function buildChickenWave(cfg) {
  ...
  for (let i = 0; i < cfg.count; i++) {
    ...
    if (Math.random() < 0.5) list.push(new SmallChicken(x));
    else list.push(new Chicken(x));
  }
  return list;
}
...
/**
 * Spawns coins at random positions within a controlled range.
 * @returns {Coin[]} List of coin objects.
 */
function buildCoins() {
  let list = [];
  for (let i = 0; i < 25; i++) {
